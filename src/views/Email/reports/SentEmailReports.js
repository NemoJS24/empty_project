...
import { ngURL, postReq } from '../../../assets/auth/jwtService'
import AdvanceServerSide from '../../Components/DataTable/AdvanceServerSide'
import FrontBaseLoader from '../../Components/Loader/Loader'
import { defaultFormatDate } from '../../Validator'
export default function SentEmailReports() {
  ...
  const getData = (currentPage = 0, currentEntry = 10, searchValue = "", advanceSearchValue = {}) => {
    ...
    setisLoading(true)
    postReq("email_template_details", form_data)
      .then((resp) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: row => <Link to={`/merchant/Email/reports/template/${row.template_unique_id}`} className='btn ' style={{ padding: "5px 10px" }}>{row?.template_template_name}</Link>, // Assuming 'name' is the property in your data for the name
      dataType: 'text',
      type: 'text',
      isEnable: true
    },
    ...
    },
    {
      ...
      cell: (row) => {
        return (<div className='d-flex gap-2'>
          ...
          <Link to={`/merchant/Email/reports/template/${row.template_unique_id}`} className='btn ' style={{ padding: "5px 10px" }}><Eye size={18} /></Link>
        </div>
        )
      },
      isEnable: true
