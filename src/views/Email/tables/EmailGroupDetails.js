...
import { Card, CardBody, Row, Modal, ModalBody, ModalHeader } from 'reactstrap'
...
export default function EmailGroupDetails() {
  ...
  const getData = (currentPage = 0, currentEntry = 10, searchValue = "", advanceSearchValue = {}) => {
    ...
    postReq(`email_group_details`, form_data)
      .then(res => {
  ...
  const uploadFile = (file) => {
    ...
    setLoader(true)
    postReq('group_import_customerEmail', form_data)
      .then(res => {
        console.log(res)
        if (res.data.success) {
  ...
  const handleDelete = () => {
    ...
    postReq('email_group_contact', form_data)
      .then(res => {
        // console.log('res:', res)
        if (res.data.success) {
          getData()
  ...
  const columns = [
    ...
    },
    {
      name: 'Email',
      minWidth: '15%',
      selector: row => row?.contact_email, // Assuming 'category' is the property in your data for the category
      type: 'select',
      isEnable: true
    }
  ]
  ...
  const customButton2 = () => {
    // return <Link to='/merchant/Email/import' className='btn btn-primary'>Add Emails </Link>
    return <div className="d-flex justify-content-center fs-5 align-items-center "><button className='btn btn-primary align-items-center' role="button"  onClick={() => toggle(2)}>Import CSV</button>
    <Link to='/merchant/Email/import'  className='btn btn-primary ms-1'>Add Emails </Link></div>
  }
  return (
    <>
      {
      ...
      <Link to='/merchant/Email/group/' className='btn btn-sm btn-primary mb-2'><ArrowLeft size={18} style={{ marginBottom: "2px" }} /> Back</Link>
      <Row >
        <Card>
          <CardBody>
