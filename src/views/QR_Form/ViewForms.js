    ...
    const columns = [
        {
            ...
            minWidth: '190px',
            selector: row => {
                return row?.Form_created_at ? moment(row?.Form_created_at).format('YYYY-MM-DD HH:mm') : ''
            },
            type: 'text',
            isEnable: true
        },
        {
            ...
            minWidth: '190px',
            selector: row => {
                const json_data = row.Form_form_json ? JSON.parse(row.Form_form_json) : {}
          ...
          },
        {
            ...
            cell: (row) => {
              return (<div className='d-flex gap-2'>
                ...
                <Link to={`/merchant/qr/reports/detail/${row.Form_id}`} className='btn ' style={{ padding: "5px 10px" }}><Eye size={18} /></Link>
              </div>
              )
            },
            isEnable: true
                ...
                <Card>
                    <CardBody>
                        <AdvanceServerSide
                            ...
                            isLoading={isLoading}
                            getData={getData}
                            count={tableData?.form_count}
                            // selectableRows={true}
                            // setSelectedRows={setSelected}
                            // selectedRows={selected}
                            // selectedContent={confirmationContent}
                            isExport={true}
                            exportUrl={`${baseURL}/qr/get_all_data/`}
                            advanceFilter={true}
                        />
                    {/* <ComTable
