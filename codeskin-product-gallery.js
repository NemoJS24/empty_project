...
<product-gallery class="product-gallery" form="{{ product_form_id }}" {% if enable_media_autoplay %}autoplay-media{% endif %} {% if enable_image_zoom %}allow-zoom="{{ max_image_zoom_level }}"{% endif %}>
    ...
    <div class="{% if mobile_controls == 'arrows' %}product-gallery__carousel-with-arrows{% else %}contents{% endif %}">
      ...
      <scroll-carousel adaptive-height id="{{ product_gallery_carousel_id }}" class="product-gallery__carousel scroll-area {% unless mobile_controls == 'arrows' %}full-bleed md:unbleed{% endunless %}" role="region">
        {%- if product != blank -%}
          ...
          {%- for media in product.media -%}
            <div class="product-gallery__media snap-center xir_product_media" data-media-type="{{ media.media_type }}" data-media-id="{{ media.id }}" role="group" aria-label="{{ 'general.accessibility.item_nth_of_count' | t: index: media.position, count: product.media.size }}" {% if filtered_indexes contains media.position %}hidden{% endif %}>
        ...
        {%- else -%}
          {%- for i in (1..3) -%}
            ...
            <div class="product-gallery__media snap-center xir_product_media {% if forloop.first %}is-initial{% endif %}" data-media-type="{{ media.media_type }}" data-media-position="{{ media.position }}" data-media-id="{{ media.id }}" role="group" aria-label="{{ 'general.accessibility.item_nth_of_count' | t: index: forloop.index, count: 3 }}">
  ...
  {%- if product.media.size > 1 or product == blank -%}
    ...
    -%}
    <input type="hidden" id="product_count" value="{{ product.media.size }}" />
    {%- if show_thumbnails_mobile or show_thumbnails_desktop -%}
      <safe-sticky class="product-gallery__thumbnail-list {% unless show_thumbnails_mobile %}hidden{% endunless %} {% unless show_thumbnails_desktop %}md:hidden{% else %}md:block{% endunless %}">
        <product-gallery-navigation align-selected aria-controls="{{ product_gallery_carousel_id }}" class="product-gallery__thumbnail-scroller bleed md:unbleed auto_parent">
          {%- if product != blank -%}
            {%- for media in product.media -%}
              <button onclick="stopCarousel('{{ forloop.index }}')" type="button" class="product-gallery__thumbnail  product_image_{{ forloop.index }}" {% if filtered_indexes contains media.position %}hidden{% endif %} data-media-type="{{ media.media_type }}" data-media-position="{{ media.position }}" data-media-id="{{ media.id }}" aria-current="{% if media == default_media %}true{% else %}false{% endif %}" aria-label="{{ 'general.accessibility.go_to_item' | t: index: forloop.index | escape }}">
...
<script>
  let itemCount = Number(document.getElementById('product_count').value)
  let currentCount = 1
  let xir_timer = 3000
  ...
  function clickCarousel() {
    ...
    document.querySelector(`.product_image_${currentCount}`).click()
    document.querySelectorAll(`.xir_product_media`).forEach((elem, index) => {
      if (currentCount === (index + 1) && (elem.getAttribute("data-media-type") === "video")) {
        console.log("elem", elem.childNodes[0].childNodes[1])
      }
    })
  }

  function startIntervalXirCarousel() {
      carouseInterval = setInterval(() => {
      
      const htmlTag = document.querySelector('html');
      if (htmlTag && htmlTag.classList.contains('lock')) {
  ...
  startIntervalXirCarousel()
  

  document.querySelector(`.auto_parent`).addEventListener('mouseenter', function(e) {
      // console.log("mouseenter")
      clearInterval(carouseInterval)
  });

  document.querySelector(`.auto_parent`).addEventListener('mouseleave', function(e) {
      // console.log("mouseleave")
      startIntervalXirCarousel()
  });

  function stopCarousel(count) {
